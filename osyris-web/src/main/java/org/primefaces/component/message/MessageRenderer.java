...
public class MessageRenderer extends UINotificationRenderer {
	...
	public void encodeEnd(FacesContext facesContext, UIComponent component)
		...
		if (target == null) {
			// CORRECTION: Do not crash when component is missing
			// throw new FacesException("Cannot find component \"" +
			// uiMessage.getFor() + "\" in view.");
			return;
		}

		Iterator<FacesMessage> msgs = facesContext.getMessages(target
		...
		if (msgs.hasNext()) {
			...
			if (!shouldRender(uiMessage, msg, severityName)) {
				...
				String styleClass = "ui-message-" + severityKey
						+ " ui-widget ui-corner-all";
				if (iconOnly) {
					styleClass = styleClass
							+ " ui-message-icon-only ui-helper-clearfix";
				}
				...
				if (!iconOnly) {
					if (uiMessage.isShowSummary()) {
						encodeText(writer, msg.getSummary(), severityKey
								+ "-summary", escape);
					}
					if (uiMessage.isShowDetail()) {
						encodeText(writer, msg.getDetail(), severityKey
								+ "-detail", escape);
					}
				}
	...
	protected void encodeText(ResponseWriter writer, String text,
			String severity, boolean escape) throws IOException {
	...
	protected void encodeIcon(ResponseWriter writer, String severity,
			String title, boolean iconOnly) throws IOException {
		writer.startElement("span", null);
